            ...
            try {
                ...
                for (Part part : parts) {
                    if (part.getSubmittedFileName() != null && !part.getSubmittedFileName().isEmpty()) {
                        // This part is a new file to upload
                        if (part.getSize() > 0) {
    ...
    private void edit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("GET".equalsIgnoreCase(method)) {
            ...
            try {
                ...
                // Extract all parts from the multipart request
                Collection<Part> parts = req.getParts();
                for (Part part : parts) {
                    if (part.getSubmittedFileName() != null && !part.getSubmittedFileName().isEmpty()) {
                        // This part is a new file to upload
                        if (part.getSize() > 0) {
                            // Use FileUtil to handle the upload logic
                            String uploadPath = FileUtil.getUploadPath(req);
                            String originalFileName = part.getSubmittedFileName();
                        ...
                        switch (fieldName) {
                            case "num":
                                updatedDto.setNum(Integer.parseInt(value));
                                break;
                            case "title":
                                updatedDto.setTitle(value);
                                break;
                            case "content":
                                updatedDto.setContent(value);
                                break;
                            case "category":
                                updatedDto.setCategory(value);
                                break;
                            case "delete_file":
                                deleteFileIdxs.add(value);
                                break;
                        }
                    }
                }
